...
% figure(5)
% subplot(5,1,1);
% plot(t,y);title('原始信号图'); %绘制原始信号图
% 
% subplot(5,1,2);
% plot(F,abs(Y));title('fft transform');
% 
% subplot(5,1,3);
% shift_f = abs(fftshift(Y));
% plot(F-f_s/2,shift_f);title('shift fft transform'); %将0频率分量(直流分量)移到坐标中心
% 
% subplot(5,1,4);
% plot(t2(length(t2)/2:length(t2)),shift_f(length(shift_f)/2:length(shift_f)));title('shift fft transform cut');%保留正频率部分
% 
% subplot(5,1,5);
% plot(F(1:length(F)/2),abs(Y(1:length(F)/2)));title('fft cut');%截取fft结果的前半部分
% 
